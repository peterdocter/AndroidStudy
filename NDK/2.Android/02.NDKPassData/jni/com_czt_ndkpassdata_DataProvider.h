/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_czt_ndkpassdata_DataProvider */

#ifndef _Included_com_czt_ndkpassdata_DataProvider
#define _Included_com_czt_ndkpassdata_DataProvider
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_czt_ndkpassdata_DataProvider
 * Method:    add
 * Signature: (II)I
 */
JNIEXPORT jint JNICALL Java_com_czt_ndkpassdata_DataProvider_add
  (JNIEnv *, jobject, jint, jint);

/*
 * Class:     com_czt_ndkpassdata_DataProvider
 * Method:    sayHelloInC
 * Signature: (Ljava/lang/String;)Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL Java_com_czt_ndkpassdata_DataProvider_sayHelloInC
  (JNIEnv *, jobject, jstring);

/*
 * Class:     com_czt_ndkpassdata_DataProvider
 * Method:    intMethod
 * Signature: ([I)[I
 */
JNIEXPORT jintArray JNICALL Java_com_czt_ndkpassdata_DataProvider_intMethod
  (JNIEnv *, jobject, jintArray);

#ifdef __cplusplus
}
#endif
#endif
